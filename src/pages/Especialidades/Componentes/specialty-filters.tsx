...
interface SpecialtyFiltersProps {
    activeFilter: SpecialtyFilter
    onFilterChange: (filter: SpecialtyFilter) => void
    counts: {
        all: number
        active: number
        inactive: number
...
const filters: { key: SpecialtyFilter; label: string; countKey: keyof SpecialtyFiltersProps["counts"] }[] = [
    { key: "all", label: "Todos", countKey: "all" },
    { key: "active", label: "Ativos", countKey: "active" },
    { key: "inactive", label: "Inativos", countKey: "inactive" },
]

export function SpecialtyFilters({ activeFilter, onFilterChange, counts }: SpecialtyFiltersProps) {
    return (
